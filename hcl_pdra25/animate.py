...
"""
from matplotlib.animation import FuncAnimation
def create_wispr_video(sequence, zero_coord, X_coord, Y_coord, Z_coord, dt=1000/5, dy=2, **kwargs):
    fig = plt.figure(figsize=(13, 8))
    global ax
    ax = fig.add_subplot(projection=sequence[0])

    wispr_norm = kwargs.get('norm', ImageNormalize(vmin=0, vmax=0.5e-11, stretch=PowerStretch(1/2.2)))
    ...
    def animate(t):
        ...
        # Plot the WISPR map
        sequence[t].plot(axes=ax, norm=wispr_norm, cmap=cmap)
        ...
        # Option to not plot axes
        if kwargs.get('plot_lmn', False):
